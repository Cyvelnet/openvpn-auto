...
function installUnbound() {
	...
	if [[ ! -e /etc/unbound/unbound.conf ]]; then
		...
		if [[ $OS =~ (debian|ubuntu) ]]; then
			...
			# Configuration
			echo 'interface: ${PrivateGatewayIP}
access-control: ${PrivateGatewayIP}/24 allow
hide-identity: yes
hide-version: yes
use-caps-for-id: yes
prefetch: yes' >>/etc/unbound/unbound.conf
			...
			# Configuration
			sed -i 's|# interface: 0.0.0.0$|interface: ${PrivateGatewayIP}|' /etc/unbound/unbound.conf
			sed -i 's|# access-control: 127.0.0.0/8 allow|access-control: ${PrivateGatewayIP}/24 allow|' /etc/unbound/unbound.conf
...
function installQuestions() {
	...
	IP=$(curl -s -X GET https://ipinfo.io/ip) 2>&1
...
keepalive 10 120
topology subnet
server ${PrivateIP} 255.255.255.0
ifconfig-pool-persist ipp.txt" >>/etc/openvpn/server.conf
	...
	2) # Self-hosted DNS resolver (Unbound)
		echo 'push "dhcp-option DNS ${PrivateGatewayIP}"' >>/etc/openvpn/server.conf
		if [[ $IPV6_SUPPORT == 'y' ]]; then
...
verb 3" >>/etc/openvpn/server.conf
	...
	# Script to add rules
	echo "#!/bin/sh
iptables -t nat -I POSTROUTING 1 -s ${PrivateIP}/24 -o $NIC -j MASQUERADE
...
ip6tables -I INPUT 1 -i $NIC -p $PROTOCOL --dport $PORT -j ACCEPT" >>/etc/iptables/add-openvpn-rules.sh
	...
	echo "#!/bin/sh
iptables -t nat -D POSTROUTING -s ${PrivateIP}/24 -o $NIC -j MASQUERADE
...
function newClient() {
	...
	cp $homeDir/$CLIENT.ovpn $homeDir/$newName-$CLIENT.ovpn
	rm $homeDir/$CLIENT.ovpn
